...
RANDOM_SEED: int = 99       # REMEMBER TO CHANGE THIS FOR EACH RUN
# Or generate it randomly
#RANDOM_SEED: int = random.randint(1, 1000000)

API_KEY: str                = "api-key"              # contact Artemy Gavrilov for API key, or generate your own at https://openai.com/api
client: OpenAI = OpenAI(api_key=API_KEY)
runner = Runner()
set_default_openai_key(API_KEY)
session = SQLiteSession(":memory:")  # In‑memory SQLite session for stateful agents, might be inneficient but no overlap
...
ENGLISH_FLUENCY_OPTIONS:     List[str] = ["Native", "Second-language", "Horrible English", "Other"]
...
def build_patient_system_prompt(
    *,
    ...
    extra_instruction: str = (
        ...
        "- Simply answer the doctor’s questions according to your personality\n"
        "- DO NOT ask your own questions.\n"
        "- Adhere to the personality traits and demographic profile provided as best as you can\n"
    ),
) -> str:
    """
...
def build_doctor_agent(*, model_settings: ModelSettings) -> Agent:
    """Return a stateful doctor agent with its system prompt."""
    return Agent(
        # client       = client,
        name         = "SimDoctor",
        model        = OPENAI_MODEL,
...
def run_patient_doctor_session(
    *,
    ...
    # deterministic agent_id (SHA‑256 of profile)   ─────────────
    import hashlib, datetime, json
    profile_key = "|".join(
        [f"{k}={v}" for k, v in sorted({**demographics, **personality}.items())]
        + [f"{s}={depression_profile[s]}" for s in sorted(depression_profile)]
        + (["EVENTS=" + ",".join(sorted(random_events))] if random_events else [])
    )
    ...
    # deterministic agent_id (UNCHANGED) ─────────────────────────
    profile_key = "|".join(
        [f"{k}={v}" for k, v in sorted({**demographics, **personality}.items())]
        + [f"{s}={depression_profile[s]}" for s in sorted(depression_profile)]
        + (["EVENTS=" + ",".join(sorted(random_events))] if random_events else [])
    ...
    doctor_agent  = build_doctor_agent(model_settings = run_cfg_doctor.model_settings)
    patient_agent = build_patient_agent(patient_system_prompt=patient_system_prompt, model_settings = run_cfg_patient.model_settings)

    # 6‑D ▸ Dialogue loop  (same business logic) ─────────────────
    conversation_history: List[Dict[str, str]] = []
    ...
    session_data = {
        ...
        "openai_call_parameters": {
            "model":       OPENAI_MODEL,
            "doctor temperature": doctor_temperature,
            "patient temperature": patient_temperature,
            "top_p":       TOP_P,
        },
